...
function qsfunctions.dispatch_and_process(msg)
    if qsfunctions[msg.cmd] then
        ...
        log(to_json(msg), 3)
        msg.lua_error = "Command not implemented: " .. msg.cmd
        msg.cmd = "lua_error"
        return msg
    end
end
...
function qsfunctions.addLabel2(msg)
    ...
    local chartTag, yValue, strDate, strTime, text, imagePath, alignment, hint, r, g, b, 
          transparency, tranBackgrnd, fontName, fontHeight = 
        unpack(spl)
...
function qsfunctions.getClassesList(msg)
    log("������� " .. msg.cmd .. ", req_id=" .. tostring(msg.req_id or "?"), 0)
...
function qsfunctions.getClientCode(msg)
    for i = 0, getNumberOf("MONEY_LIMITS") - 1 do
        local cc = getItem("MONEY_LIMITS", i).client_code
        if cc then
            msg.data = cc
            return msg
        end
...
function qsfunctions.getTradeAccount(msg)
    for i = 0, getNumberOf("trade_accounts") - 1 do
        local ta = getItem("trade_accounts", i)
        if ta.class_codes:find("|" .. msg.data .. "|", 1, true) then
...
-- ------------------------------------------------------------------------------
-- ��������� ������� (paramRequest, getParamEx, getDepo � �.�.) 
-- �������� ��� ���������, �����������, ��� ��� � ���� ��� ��������� �����������
-- ------------------------------------------------------------------------------
...
function qsfunctions.get_candles(msg)
    ...
    -- Получаем свечи из графика
    local t, n, l = getCandlesByIndex(tag, line, first_candle, count)
    
    log("Получено свечей из QUIK: " .. tostring(n), 2)

    local candles = {}
    if t and n > 0 then
        -- ВАЖНО: цикл от 1 до n
        for i = 1, n do
            local c = t[i-1] -- getCandlesByIndex возвращает таблицу с 0-индексом внутри
            if c then
...
function qsfunctions.get_candles_from_data_source(msg)
	...
	if not is_error then
        ...
        local spl = split(msg.data.data, "|")
       	local count = tonumber(spl[5]) --- возвращаем последние count свечей. Если равен 0, то возвращаем все доступные свечи.
        local class, sec, interval = get_candles_param(msg)
		local candles = {}
...
function data_source_callback(index, class, sec, interval)
    ...
    -- Проверяем, что индекс изменился и источник данных существует
    if data_sources[key] and index ~= last_indexes[key] then
        last_indexes[key] = index
 
        -- Получаем предыдущую (закрытую) свечу
        local candle = fetch_candle(data_sources[key], index - 1)
        
        if candle then
            -- Формируем чистый объект без типов данных QUIK (userdata)
            local clean_candle = {
            ...
            local msg = {
                t    = timemsec(),
                cmd  = "NewCandle",
                data = clean_candle  -- Отправляем именно очищенные данные
            }
...
function qsfunctions.subscribe_to_candles(msg)
    ...
    if not is_error and ds then
        ...
        data_sources[key] = ds
        -- Устанавливаем текущий индекс свечи
        last_indexes[key] = ds:Size() 
 
        -- Используем стандартные названия методов QUIK (строчные буквы)
        ds:SetUpdateCallback(function(idx)
            data_source_callback(idx, class, sec, interval)
        end)
        msg.data = "success"
        ...
        msg.data = "error"
        msg.lua_error = "Подписка не удалась"
        log("Ошибка подписки: " .. (msg.lua_error or "неизвестно"), 3)
    end
    --log("Содержимое таблицы: " .. json.encode(msg), 2)
    return msg
end
