...
-- Читает элемент системной таблицы client_codes: в отличие от money_limits/trade_accounts
-- (записи-таблицы с полями), элементы client_codes бывают ЛИБО простой строкой (сам код),
-- ЛИБО таблицей с полем client_code/code — обрабатываем оба варианта.
...
-- Резервный источник (старое поведение): раньше здесь ошибочно стояло "MONEY_LIMITS"
-- (регистр не совпадает с реальным именем системной таблицы QUIK Lua — везде в этом файле
-- используется "money_limits", см. qsfunctions.getMoneyLimits и др.), из-за чего
-- getNumberOf/getItem не находили нужную таблицу и по факту всегда отдавали максимум одну
-- запись. Основной источник теперь — таблица "client_codes" (даёт все коды клиента сразу,
-- без привязки к валюте/лимиту), money_limits оставлен как fallback на случай, если
...
function qsfunctions.getClientCode(msg)
    ...
    -- client_codes пуста/недоступна — пробуем старый источник
    local seen, codes = {}, {}
